...
const JobList = () => {
  ...
  const GetJobPosts = async (page = 1, search = "") => {
    try {
      const res = await axios.get("https://server.hirix.pk/get-postsBYAdmin", {
        params: { page, search },
    ...
    if (clientId) {
      try {
        const res = await axios.get(
          `https://server.hirix.pk/getPostSpecific/${clientId}`,
          { headers: { "x-access-token": token } }
        );
  ...
  const Delete = async (id) => {
    try {
      const res = await axios.delete(`https://server.hirix.pk/deleteJob/${id}`, {
        headers: { "x-access-token": token },
      });
      alert(res.data.msg);
