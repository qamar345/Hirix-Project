...
<body>
  ...
  // Same resilient fallback chain as the other transactional emails - this
  // used to build its own one-shot transporter with no fallback, so it was
  // exposed to the exact "slow/failing first hop = lost email" issue that
  // was fixed for verification/reset mail (see mailer-controller history).
  return sendMailWithFallback(mailOptions);
};
